...
const DashboardParticipantes = ({ children }) => {
  ...
  // Fix: respetar showUnique para contar IDs únicos cuando corresponde
  const participantesGlobal = useMemo(() => {
    if (showUnique) {
      const noAcciones = columnsMap.acciones.no;
      const noProcesos = columnsMap.procesos.no;
      const ids = new Set(
        [...acciones, ...procesos]
          .map(d => d[noAcciones] || d[noProcesos])
          .filter(Boolean)
      );
      return ids.size;
    }
    return acciones.length + procesos.length;
  }, [acciones, procesos, showUnique]);
