  ...
  const getItemValue = useCallback(
    (item, f) => {
      if (f === "mes") {
        ...
        const value = extractMonth(item[resolvedCol]);
        return value;
      }
      ...
      const getValue = (type) => {
        ...
        const value = item[resolvedColumn];
        return value;
      };
...
const Filters = ({
  ...
  const visibleFields = useMemo(
    () =>
      filterFields.filter(({ onlyTabs }) => {
        if (!onlyTabs) return true;
        if (tab === "todo") return onlyTabs.includes("acciones") && onlyTabs.includes("procesos");
        return onlyTabs.includes(tab);
      }),
    [filterFields, tab]
  );
